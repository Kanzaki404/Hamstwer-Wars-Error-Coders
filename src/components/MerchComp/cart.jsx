...
function Cart({ open, onClose, cartItem, allItem }) {
  ...
  function deleteItem(id) {
    console.log(id);
    cartProducts.splice(
      cartProducts.findIndex(function (i) {
        return i.id === id;
      }),
      1
    );
  ...
  const selectedCartItem = cartProducts.map((e) => (
    <li key={e.id + Math.floor(Math.random() * 1000000)}>
      <img src={e.im} alt="Tshirt" />
      <span />
      <button type="submit" onClick={() => deleteItem(e.id)}>
        Remove{e.id}
      </button>
    </li>
  ));
        ...
        <div className="secondDiv">
          ...
          <div className="secondDivFrame">
            <form>
              ...
              <br />
              <br />
              <button className="btnMain" onClick={onClose}>
                CheckOut<div className="btnDiv"></div>
                <img className="btnI" src={imageClose}></img>
              </button>
            </form>
          </div>
        </div>
